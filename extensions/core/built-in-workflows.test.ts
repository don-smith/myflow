...
import { createMockPi, createMockSessionChain, mockAssistantMessage } from "@myflow/test-utils";
import {
	acts,
	defineRoute,
	defineWorkflow,
...
import { describeFlow, fs as fsHandle } from "@myflow/workflow/registration";
...
const deriveAndValidate = (
	wf: Workflow,
	opts?: { skillContracts?: Map<string, import("@myflow/workflow/registration").SkillContract> },
) => {
...
const withDerivedOutcomes = (
	wf: Workflow,
	skillContracts?: Map<string, import("@myflow/workflow/registration").SkillContract>,
): Workflow => {
...
describe("readers must not silently drop the first row when no header is on disk", () => {
	...
	beforeEach(() => {
		tmpDir = mkdtempSync(join(tmpdir(), "myflow-i2-repro-"));
	});
	...
	it("readLastStage returns the row even when the header line is missing", async () => {
		const { readLastStage } = await import("@myflow/workflow");
...
describe("truncated reply (stopReason=length) must not record as completed", () => {
	...
	beforeEach(() => {
		tmpDir = mkdtempSync(join(tmpdir(), "myflow-i7-repro-"));
	});
...
describe("recordStage signals success and advances stageNumber monotonically", () => {
	...
	beforeEach(() => {
		tmpDir = mkdtempSync(join(tmpdir(), "myflow-i3-repro-"));
	...
	it("returns the assigned stageNumber on a successful write", async () => {
		const { recordStage } = await import("@myflow/workflow/internal");
...
describe("non-first stage with no artifactPath halts instead of reusing originalInput", () => {
	...
	beforeEach(() => {
		tmpDir = mkdtempSync(join(tmpdir(), "myflow-q7-repro-"));
	});
...
describe("phase fanout rows preserve both stage name (record key) and skill body across aliasing", () => {
	...
	beforeEach(() => {
		tmpDir = mkdtempSync(join(tmpdir(), "myflow-i9-repro-"));
	});
...
describe("vet workflow", () => {
	...
	describe("backward-jump loop behavior", () => {
		...
		beforeEach(() => {
			tmpDir = mkdtempSync(join(tmpdir(), "myflow-q4-loop-"));
		});
...
describe("polish workflow", () => {
	...
	describe("integration", () => {
		let tmpDir: string;
		beforeEach(() => {
			tmpDir = mkdtempSync(join(tmpdir(), "myflow-polish-"));
		});
		afterEach(() => {
			rmSync(tmpDir, { recursive: true, force: true });
		});
...
// bundled by @myflow/pi) and rides the unexercised continue path. Kept here as a
...
describe("ship workflow", () => {
	...
	describe("FRONTMATTER_PHASE_FANOUT", () => {
		let tmpDir: string;
		beforeEach(() => {
			tmpDir = mkdtempSync(join(tmpdir(), "myflow-ship-"));
		});
		afterEach(() => {
			rmSync(tmpDir, { recursive: true, force: true });
		});
	...
	describe("end-to-end via runWorkflow", () => {
		let tmpDir: string;
		beforeEach(() => {
			tmpDir = mkdtempSync(join(tmpdir(), "myflow-ship-e2e-"));
		});
		afterEach(() => {
			rmSync(tmpDir, { recursive: true, force: true });
		});
...
describe("polish — REVIEW_PHASE_ITERATE (frontmatter-driven)", () => {
	...
	let tmpDir: string;
	beforeEach(() => {
		tmpDir = mkdtempSync(join(tmpdir(), "myflow-polish-iter-"));
	});
	afterEach(() => {
		rmSync(tmpDir, { recursive: true, force: true });
	});
