/**
 * Built-in workflows shipped with @myflow/pi. Each workflow's `stages`
 ...
 *
 * Route edges use `gate(...)` from `@myflow/workflow`, which
 * attaches `.targets` metadata so reachability checks and graph
 * introspectors can enumerate possible branches without probing.
 *
 * These workflows name skills bundled by @myflow/pi (research, design, plan,
 * implement, validate, code-review, revise, commit). Installing
 * workflow without @myflow/pi means these workflows aren't loaded —
...
import {
	...
	type RunState,
	type Workflow,
} from "@myflow/workflow/registration";
import { StagePreflightError } from "@myflow/workflow/runner";
 ...
 *
 * Cap: a plan declaring more than 32 phases throws. The @myflow/pi planning skills
