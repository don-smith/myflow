/**
 * myflow-models/command — /myflow-models cascade picker command handler.
 *
...
import { loadJsonConfig, modelKey, saveJsonConfig } from "@myflow/config";
import {
	CONFIG_PATH,
...
/** Guard: /myflow-models needs an interactive UI. Notifies + returns false if not. */
...
async function resetAllOverrides(ctx: ExtensionContext): Promise<void> {
	// Gate behind a confirm dialog, mirroring /myflow-setup's prune (the repo's
