/**
 * Contract-derived outcome resolver for @myflow/pi's built-in workflows.
 *
 ...
 *
 * Registered via `registerOutcomeDeriver` (from `@myflow/workflow/startup`)
 * so the loader invokes it after `buildEffectiveContracts` and before the
 * validation loop — see `load/index.ts`.
 */

import type { OutcomeDeriverFn } from "@myflow/workflow/registration";
...
/**
 * Register the outcome deriver with workflow's loader. Called from
 * `registerSkillContractsSource()` so the deriver is available before the
 * first `/wf load`. Dynamic-imports workflow (sibling may be absent).
 */
export async function registerOutcomeDerivation(): Promise<void> {
	try {
		const { registerOutcomeDeriver } = await import("@myflow/workflow/startup");
