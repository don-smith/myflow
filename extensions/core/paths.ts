/**
 * Resolved filesystem paths for @myflow/pi's own bundled resources.
 *
 * `PACKAGE_ROOT` is computed at module load from this file's URL. The walk-up
 * is anchored to this file's location (`extensions/core/paths.ts`) — three
 * `dirname` levels reach the @myflow/pi package root. Other resource directories
...
export const PACKAGE_ROOT = (() => {
	...
	// extensions/core/paths.ts -> @myflow/pi/
 ...
 * logs a `[@myflow/pi]`-prefixed warning so the failure is diagnosable. Without
...
export function loadBundledSkillNames(dir: string): ReadonlySet<string> {
	...
	try {
		...
		console.warn(`[@myflow/pi] could not enumerate bundled skills under ${dir}: ${reason}`);
	...
	if (names.size === 0) {
		console.warn(
			`[@myflow/pi] bundled skills directory ${dir} contains no subdirectories — ` +
