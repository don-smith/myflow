/**
 * Session lifecycle wiring for core.
 *
...
const msgAgentsAdded = (n: number) => `Copied ${n} @myflow/pi agent(s) to ~/.pi/agent/agents/`;
const msgAgentsHealed = (parts: string[]) => `Synced bundled agent(s): ${parts.join(", ")}.`;
const msgAgentsDrift = (parts: string[]) => `${parts.join(", ")} agent(s). Run /myflow-update-agents to sync.`;
const msgAgentsErrors = (n: number) => `Agent sync reported ${n} error(s). Run /myflow-update-agents for details.`;
function msgMissingSiblings(pkgs: string[]): string {
	const n = pkgs.length;
	const title = `@myflow/pi: ${n} sibling extension${n === 1 ? "" : "s"} missing`;
	const body = [...pkgs.map((p) => `• ${p}`), "", "Run /myflow-setup to install them."];
...
// Runs every fire — `myflow: <skill>` is a per-stage display string (each
...
async function onBeforeAgentStart(
	...
	if (parsed && isOwnedSkill(parsed.name)) ctx.ui.setStatus("myflow-skill", `myflow: ${parsed.name}`);
...
async function onAgentEnd(_event: AgentEndEvent, ctx: ExtensionContext): Promise<void> {
	ctx.ui.setStatus("myflow-skill", undefined);
}
...
// Allowlist of @myflow/pi's own skill names, sourced from the shared
...
function migrateThoughtsToArtifacts(cwd: string): void {
	...
	try {
		...
		for (const entry of entries) {
			...
			if (!existsSync(dest)) {
				console.warn(`[@myflow/pi] migration: failed to copy ${src} → ${dest}`);
		...
		console.warn(`[@myflow/pi] migration: ${e instanceof Error ? e.message : String(e)}`);
