...
describe("SIBLINGS registry", () => {
	...
	it("does NOT list btw (standalone-only — @myflow/pi has no runtime dependency on it)", () => {
		expect(SIBLINGS.find((s) => s.pkg === "npm:@myflow/btw")).toBeUndefined();
	});
	...
	it("args does NOT match args-extended (word boundary)", () => {
		const argsEntry = SIBLINGS.find((s) => s.pkg.endsWith("/args"));
		expect(argsEntry).toBeDefined();
		expect(argsEntry?.matches.test("@myflow/args-extended")).toBe(false);
	});

	it("i18n does NOT match i18n-utils (word boundary)", () => {
		const i18nEntry = SIBLINGS.find((s) => s.pkg.endsWith("/i18n"));
		expect(i18nEntry).toBeDefined();
		expect(i18nEntry?.matches.test("@myflow/i18n-utils")).toBe(false);
		expect(i18nEntry?.matches.test("@myflow/i18n")).toBe(true);
	});
	...
	it("pi-subagents SIBLINGS[0] matches BOTH the @tintinweb/pi-subagents fork and the @gotgenes/pi-subagents fork", () => {
		...
		// Upstream fork (default install target via /myflow-setup)
	...
	it("pi-subagents SIBLINGS[0] regex does NOT match unrelated @-scoped pi-subagents-* variants", () => {
		...
		// such as `@myflow/pi-subagents-utils` (a dash right after the
