...
import { harvestStageContracts } from "@myflow/workflow/internal";
import type { ConsumesSpec, ProducesSpec } from "@myflow/workflow/registration";
...
describe("bundled skill contracts", () => {
	...
	it("registerSkillContractsSource registers the plans composition comparator eagerly", async () => {
		const { getCompositionComparators } = await import("@myflow/workflow/internal");
...
describe("registerSkillContractsSource", () => {
	describe("when the workflow sibling is absent", () => {
		afterEach(() => {
			vi.doUnmock("@myflow/workflow/startup");
			vi.resetModules();
		});

		it("no-ops without throwing", async () => {
			vi.resetModules();
			vi.doMock("@myflow/workflow/startup", () => {
				throw Object.assign(new Error("Cannot find package '@myflow/workflow'"), {
