...
All notable changes to `@myflow/ask-user-question` are documented here.
...
### Changed
- `ask_user_question` now selects the negotiated Pi ACP input carrier only when its exact bridge capability is present; incompatible RPC clients fail visibly.
...
### Fixed
- The Pi TUI questionnaire now uses the normal editor layout instead of covering the transcript with an overlay. Tall dialogs leave six terminal rows for the transcript and Pi footer.
...
### Added
- `rpiv:ask-user:prompt` event emitted before showing the questionnaire, with a typed payload and importable event contract from `@myflow/ask-user-question/events`.
...
### Changed
- `@myflow/i18n` is now a soft optional peer (`peerDependenciesMeta.optional: true`). The bridge and module-init `registerStrings` call use a dynamic `await import()` inside try/catch, so a standalone install of just `ask-user-question` no longer fails with `Cannot find module '@myflow/i18n'` — the dialog stays online with English-only UI when the SDK isn't present, and lights up localization automatically when it is.
...
### Added
- Localized TUI strings via `@myflow/i18n`: sentinel rows ("Type something.", "Chat about this", "Next"), submit/cancel labels, dialog hints, review-tab heading and prompts, preview placeholder, notes affordance, and the chat-summary line all resolve at render time through the i18n bridge.
- Ships Deutsch / English / Español / Français / Português (PT) / Português (BR) / Русский / Українська translation maps under `locales/`. Auto-translated drafts marked in each file's `_meta.notes` — native-speaker contributions welcome (see i18n README → "Contributing translations").
...
### Changed
- Consolidated into the `don-smith/myflow` monorepo. Version aligned to the @myflow/pi family lockstep starting point. No runtime behavior change from `0.1.4`.
