...
describe("loadJsonConfig", () => {
	it("returns {} for missing file", () => {
		const result = loadJsonConfig(tmpConfigPath("myflow-test-load"));
		expect(result).toEqual({});
	});

	it("parses valid JSON object", () => {
		const path = tmpConfigPath("myflow-test-load");
...
describe("saveJsonConfig", () => {
	it("returns true on success and writes formatted JSON with trailing newline", () => {
		const path = tmpConfigPath("myflow-test-save");
	...
	it("creates parent directories recursively", () => {
		const path = join(TMP_BASE, "myflow-test-save", "nested", "config.json");
