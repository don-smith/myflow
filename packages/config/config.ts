/**
 * config — shared config I/O utilities for myflow sibling packages.
 *
...
export function loadJsonConfig<T>(path: string): T {
	...
	try {
		...
		// the user's HOME.
		console.warn(`config: invalid JSON at ${path}, using default ({}) — ${(err as Error).message}`);
		return {} as T;
	}
}
 ...
 *
 * Every current @myflow/* save call site is user-initiated and shows a
...
// into config. The runtime invariants (non-empty string, non-empty
 ...
 * Byte-identical logic previously in todo, ask-user-question,
 * advisor, and web-tools.
 */
 ...
 *   - "provider:modelId" — legacy form (persisted by released advisor
