...
<div align="center">
  <a href="https://github.com/don-smith/myflow/tree/main/packages/i18n">
    <picture>
      <img src="https://raw.githubusercontent.com/don-smith/myflow/main/packages/i18n/docs/cover.png" alt="i18n cover" width="50%">
    </picture>
  </a>
</div>
...
- **Zero-import escape hatch** - `globalThis[Symbol.for("i18n")]` exposes a frozen `{ locale, namespaces }` snapshot for tools that prefer not to depend on this package.
...
Then restart Pi. (If you installed via `pi install npm:@myflow/pi` + `/myflow-setup`, this is already done - `/myflow-setup` auto-wires every sibling.)
...
import { registerStrings, scope } from "@myflow/i18n";
...
The inline example above is fine for a one-key smoke test. For a real Pi extension, use the file-based pattern that scales to dozens of strings and ten contributors. Two production exemplars in this monorepo follow this exact shape: `packages/ask-user-question/` (questionnaire UI; bridge owns `t` + `displayLabel(kind)` for sentinel rows) and `packages/todo/` (todo overlay + `/todos` command; bridge owns `t` + `formatStatusLabel(status)` reused across overlay and command). Read either alongside this guide.
...
Mark it `optional: true` in `peerDependenciesMeta` so npm doesn't warn when a user installs your extension standalone without i18n. Pair this with the dynamic-import shim shown in step 3 - your extension stays online with English-only UI when the SDK isn't installed, and lights up localization automatically when it is.
...
try {
  const sdk = (await import("@myflow/i18n")) as I18nSDK;
...
**Why dynamic import instead of static `import { scope } from "@myflow/i18n"`?** A static ESM import is hoisted and evaluated at module load - if the SDK isn't on disk, your entire extension fails to load with `Cannot find module '@myflow/i18n'`. The dynamic `await import()` inside a try/catch lets module load proceed, and the identity-fallback closure keeps your render call sites working with English. Top-level await is required because the `t` export is consumed synchronously by every render call site downstream.
...
pi install npm:@myflow/i18n           # if not already installed
...
Add the locale to the `/languages` picker by appending an entry to `SUPPORTED_LOCALES` in `packages/i18n/i18n.ts`:
...
3. The `SUPPORTED_LOCALES` entry in `i18n/i18n.ts`.
...
For tools that prefer not to import this package, the active state is also published at `globalThis[Symbol.for("i18n")]`:

```ts
const I18N = Symbol.for("i18n");
...
[![npm version](https://img.shields.io/npm/v/@myflow/i18n.svg)](https://www.npmjs.com/package/@myflow/i18n)
