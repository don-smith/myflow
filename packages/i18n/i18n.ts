 ...
 *
 * Public surface for consumers (@myflow/* family or third-party Pi extensions):
...
//
// All SDK state is anchored on globalThis under `Symbol.for("i18n.runtime")`
...
// a consumer's `node_modules/@myflow/i18n` import — Pi's TS loader
// caches them under different paths and each `import { tr }` would otherwise
// see its own private copy of `activeStrings`).
//
// The public read-only snapshot under `Symbol.for("i18n")` is kept as a
