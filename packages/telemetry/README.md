...
<div align="center">
  <a href="https://github.com/don-smith/myflow/tree/main/packages/telemetry">
    <picture>
      <img src="https://raw.githubusercontent.com/don-smith/myflow/main/packages/telemetry/docs/cover.png" alt="telemetry cover" width="50%">
    </picture>
  </a>
</div>

MLflow observability for [Pi Agent](https://github.com/badlogic/pi-mono). `telemetry` auto-instruments Pi lifecycle events and sub-agent activity and dispatches them to one or more configured providers through a bounded async pipeline that never blocks the agent.
...
```bash
pi install npm:@myflow/telemetry
```
...
The minimal local MLflow that `telemetry` can talk to is a single container with **proxied artifacts** and the security middleware in permissive mode for local dev. Works identically on Docker Desktop and OrbStack.
