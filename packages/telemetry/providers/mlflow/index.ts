...
export class MlflowProvider implements TelemetryProvider {
	...
	async trackEvent(event: TelemetryEvent): Promise<void> {
		...
		try {
			...
			if (this.failedKinds.has(event.kind)) {
				...
				console.warn(`[telemetry] mlflow provider recovered for kind=${event.kind}`);
			}
		} catch (err) {
			if (!this.failedKinds.has(event.kind)) {
				this.failedKinds.add(event.kind);
				console.warn(
					`[telemetry] mlflow provider error on kind=${event.kind}: ${err instanceof Error ? err.message : String(err)}`,
				);
			}
		}
	}
	...
	async flush(): Promise<void> {
		try {
			...
			console.warn(`[telemetry] mlflow flush error: ${err instanceof Error ? err.message : String(err)}`);
		}
	}
	...
	private ensureInit(): void {
		...
		if (!resolved.trackingUri) {
			console.warn(
				"[telemetry] mlflow provider registered but MLFLOW_TRACKING_URI is not configured — events will be silently dropped",
			);
			return;
		}
		...
		try {
			...
			console.warn(
				`[telemetry] mlflow init failed; events will be dropped: ${err instanceof Error ? err.message : String(err)}`,
			);
		}
	}
}
