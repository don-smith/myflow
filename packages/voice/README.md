...
<div align="center">
  <a href="https://github.com/don-smith/myflow/tree/main/packages/voice">
    <picture>
      <img src="https://raw.githubusercontent.com/don-smith/myflow/main/packages/voice/docs/cover.png" alt="voice cover" width="50%">
    </picture>
  </a>
</div>

Talk to [Pi Agent](https://github.com/badlogic/pi-mono) instead of typing. `voice` adds the `/voice` slash command — open the overlay, speak, hit `Enter`, and your transcript drops straight into Pi's editor. Speech-to-text runs **entirely on your machine** via [sherpa-onnx](https://github.com/k2-fsa/sherpa-onnx) Whisper (base multilingual int8). No cloud, no API keys, no telemetry.

![Voice dictation overlay above the Pi editor](https://raw.githubusercontent.com/don-smith/myflow/main/packages/voice/docs/overlay.jpg)
...
`voice` is **opt-in** — it's not part of `/myflow-setup` because the native deps (sherpa-onnx, decibri) are heavyweight. Install it directly:

```sh
pi install npm:@myflow/voice
```
...
Install `@myflow/i18n` alongside it to flip the overlay, status bar, and settings strings to your active locale:
...
The microphone is the OS default input — `voice` does not expose device selection. The bundled Whisper base multilingual model is loaded from `~/.pi/models/whisper-base/`; alternative models aren't supported today.
...
- [`@myflow/i18n`](https://www.npmjs.com/package/@myflow/i18n) — localizes the `/voice` overlay UI.
- [`@myflow/pi`](https://www.npmjs.com/package/@myflow/pi) — umbrella + `/myflow-setup` for the rest of the `@myflow/*` family.

## License

[![npm version](https://img.shields.io/npm/v/@myflow/voice.svg)](https://www.npmjs.com/package/@myflow/voice)
