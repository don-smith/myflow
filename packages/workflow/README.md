...
<div align="center">
  <a href="https://github.com/don-smith/myflow/tree/main/packages/workflow">
    <picture>
      <img src="https://raw.githubusercontent.com/don-smith/myflow/main/packages/workflow/docs/cover.png" alt="workflow cover" width="50%">
    </picture>
  </a>
</div>
...
**Skill-agnostic.** The runner sends `/skill:<name>` via Pi's native dispatch — it doesn't know or care who shipped the skill. Install on its own and write workflows over your own `~/.pi/agent/skills/`, or pair with [`@myflow/pi`](../@myflow/pi) to use @myflow/pi's bundled `ship`, `build`, `arch`, and `vet` workflows over @myflow/pi's bundled skills.
...
- **Config file** — the one TypeScript file you hand-edit. Accepts three default-export shapes:
  ...
  ```ts
  // 1. A single Workflow
  import { defineWorkflow, produces, acts } from "@myflow/workflow";
...
```ts
import { gate, gt, eq } from "@myflow/workflow";
...
```ts
import { produces, fs, type ScriptContext } from "@myflow/workflow";
...
```ts
import { acts, type ScriptContext } from "@myflow/workflow";
import { readFile, writeFile } from "node:fs/promises";
import { join } from "node:path";
...
```ts
import { terminal, type ScriptContext } from "@myflow/workflow";
...
```ts
import { registerBuiltIns, type WorkflowHost } from "@myflow/workflow";
...
```ts
import { registerLifecycle, type WorkflowHost } from "@myflow/workflow";
...
The registry is anchored on `Symbol.for("@myflow/workflow:lifecycle")`, mirroring the `registerBuiltIns` pattern, so cross-package module resolution still shares one slot. Snapshot semantics: each event observes the registry as it stands at that instant — a registration made mid-event applies to subsequent events, not the in-flight one.

`@myflow/pi` is the reference consumer (overlay widget that visualises in-flight runs).

## Host boundary

`workflow`'s public type surface names **zero** `@earendil-works/pi-coding-agent`
...
```ts
import { runWorkflow } from "@myflow/workflow";
...
```ts
import { runWorkflowByName } from "@myflow/workflow";
...
```ts
import { resumeWorkflowByRunId, listRuns } from "@myflow/workflow";
...
There is no framework default for `produces` — load-time validation rejects a stage without an outcome. The `.myflow/artifacts/<bucket>/<file>.md` layout is an rpiv convention, not a framework truth; pair with [`@myflow/pi`](../@myflow/pi) for `rpivArtifactMdOutcome`, or wire your own.
...
```ts
import { defineCollector, opaque, type Artifact } from "@myflow/workflow";
...
```ts
import { defineCollector, fs, gitHeadSnapshot, type GitHeadSnapshot } from "@myflow/workflow";
...
Format-specific parsers (markdown frontmatter, YAML, TOML, …) live in the convention layer that owns them — @myflow/pi ships its own `frontmatterParser`.
