...
Every workflow file is TypeScript, loaded via `jiti` (no build step required). Import the authoring DSL from `@myflow/workflow`:

```typescript
import { defineWorkflow, produces, acts, gate, gt, eq } from "@myflow/workflow";
```
