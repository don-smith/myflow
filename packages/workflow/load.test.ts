...
// (@myflow/pi, etc.) happens to be registering workflows. Reset per test.
...
`,
	...
	it("returns default=undefined when no layer registered any workflows", async () => {
		// Standalone workflow install (no @myflow/pi, no overlays). The
...
describe("loadWorkflows — legacy .workflow/ overlay", () => {
	it("warns when a legacy .workflow/ directory exists and does NOT load it", async () => {
		// Author a workflow under the OLD dashed path. It must be ignored.
		const legacyRoot = join(TEST_TMP, ".workflow");
...
`,
	...
	it("emits no legacy notice when no .workflow/ directory exists", async () => {
		const loaded = await loadWorkflows(TEST_TMP);
		expect(loaded.issues.some((i) => i.kind === "load" && /\.workflow/.test(i.message))).toBe(false);
	});
