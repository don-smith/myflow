 ...
 *
 * Scope is the framework runtime; @myflow/pi conventions stay out of this
...
import { createMockSessionChain, mockAssistantMessage } from "@myflow/test-utils";
...
describe("state.named — publish key resolution + accumulation", () => {
	let tmpDir: string;
	beforeEach(() => {
		tmpDir = mkdtempSync(join(tmpdir(), "myflow-named-"));
	});
	afterEach(() => {
...
describe("reads: prompt format", () => {
	let tmpDir: string;
	beforeEach(() => {
		tmpDir = mkdtempSync(join(tmpdir(), "myflow-reads-"));
	});
	afterEach(() => {
		rmSync(tmpDir, { recursive: true, force: true });
	});
...
describe("reads: runtime preflight halts when slot is empty", () => {
	let tmpDir: string;
	beforeEach(() => {
		tmpDir = mkdtempSync(join(tmpdir(), "myflow-named-halt-"));
	});
	afterEach(() => {
		rmSync(tmpDir, { recursive: true, force: true });
	});
