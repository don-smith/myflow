	...
	dispatch(event: TelemetryEvent): void {
		...
		if (this.queue.length >= maxQueueSize) {
			...
			if (!this.backpressureActive) {
				...
				console.warn(`[telemetry] backpressure: queue saturated at ${maxQueueSize}; dropping oldest events`);
			}
			...
			console.warn("[telemetry] backpressure recovered: queue back under capacity");
		}
		...
		results.forEach((result, idx) => {
			...
			if (result.status === "rejected") {
				if (!this.failedProviders.has(name)) {
					...
					console.warn(`[telemetry] provider ${name} rejected event: ${reason}`);
				}
			} else if (this.failedProviders.has(name)) {
				this.failedProviders.delete(name);
				console.warn(`[telemetry] provider ${name} recovered`);
			}
		});
	}
}
