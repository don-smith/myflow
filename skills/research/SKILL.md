...
`$ARGUMENTS` — free-text research prompt, or a `.myflow/artifacts/discover/*.md` path to chain from discover.
...
1. **Compile findings:**
   ...
   - Use `.myflow/artifacts/` findings as supplementary historical context
...
1. **Determine metadata** (from the Metadata block above):
   - Filename: `.myflow/artifacts/research/<slug>_<topic>.md` — `<slug>` is the second tab-separated field on line 1 of the Metadata block above; `<topic>` is a brief kebab-case description.
...
2. **Write the research document** — this document is compressed context for a new session. Include everything the planner needs to make architectural decisions without re-researching:
   ...
   **Lessons from docs**:
   - .myflow/artifacts/path/to/doc.md — key lesson extracted
   ...
   **Takeaway**: {one sentence — what to watch out for}
   ...
   ## Historical Context (from `.myflow/artifacts/`)
   {Links only — one line per doc, no summaries of their contents}
   - `.myflow/artifacts/something.md` — {one-line description of what this doc covers}
   ## Developer Context
...
`.myflow/artifacts/research/{filename}.md`
