...
Before substantive Verify work, inspect lifecycle feedback coverage. If the completed Implement attempt is pending, show its one deferred question using the exact four choices in the shared stage checkpoint. Record `feedback-requested` with `--attempt-id <implement-attempt-id>`, then call `node <myflow-package-root>/skills/observing-myflow/scripts/record-stage-feedback.mjs` for that same Implement attempt. Record the result with `feedback-recorded --attempt-id <implement-attempt-id>` and only the returned status and private reference. Structured interaction is preferred and plain-text is the fallback. A feedback failure does not block verification or the stage transition; leave pending coverage visible and continue.
